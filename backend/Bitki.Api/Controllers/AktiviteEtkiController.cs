...
namespace Bitki.Api.Controllers
{
    ...
    public class AktiviteEtkiController : ControllerBase
    {
        ...
        public AktiviteEtkiController(IAktiviteEtkiRepository repository) { _repository = repository; }
        ...
        public async Task<ActionResult<int>> Create([FromBody] AktiviteEtki entity) { var id = await _repository.AddAsync(entity); return CreatedAtAction(nameof(GetById), new { id }, id); }

        [HttpPut("{id}")]
        [Authorize(Roles = "Admin")]
        public async Task<IActionResult> Update(int id, [FromBody] AktiviteEtki entity) { if (id != entity.Id) return BadRequest("ID mismatch"); await _repository.UpdateAsync(entity); return NoContent(); }
